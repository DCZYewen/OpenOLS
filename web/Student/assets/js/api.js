...
function setCourseName(token, userid, courseid) { // This func works but shitty. - mzWyt
    ...
    $.get(url,function callback(data){
        if (data.status == 'OK'){
            document.getElementById("last_class").innerHTML = html + data.title + '</a>';
        }
        else{
            alert("获取课程信息失败！");
        }
...
function load_mainpage_info() {
    ...
    $.get(url,function callback(data){
        if (data.status == 'OK'){
            ...
            setCourseName(Token, User_Id, data.information.last_course);   
